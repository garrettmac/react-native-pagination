...
else  return  <View style={[{width:(horizontal===true)?width:40,height:(horizontal===false)?height:30,}]}>{React.cloneElement(loadingComponent, {style})}</View>
...
if(paginationVisibleItems.length)paginationVisibleItemsIndexList=paginationVisibleItems.map(i=>i.index)
 ...
 let paginationVisiblePadItems= paginationVisableItemsIndexArray.map((o,i) => Object.assign({},{index:_.get(paginationItems,`[${o}].index`),key:_.get(paginationItems,`[${o}].index`),item:_.get(paginationItems,`[${o}]`,{}),isViewable:false}))
...
<View>{React.cloneElement(this.props.startDot, {onPress:()=>{try {this.props.refs.scrollToItem({index: 0,key:0,item:paginationItems[0],isViewable:true})} catch (e) {console.warn(" e: ",e)}}})}</View>
          ...
          {flatListPaginationItems.map((item,i) => {
            ...
            return (<View style={{flex:1}} key={i}>{React.Children.map(renderDot(item,i),child =>React.cloneElement(child, {  horizontal}))}</View>)
          })}
