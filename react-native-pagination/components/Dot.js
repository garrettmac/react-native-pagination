...
const Dot = props => {
  let {
    ...
    dotIconNameNotActive,
    dotIconNameActive,
    dotIconNameEmpty,
    dotIconSizeNotActive,
    dotIconSizeActive,
    dotIconSizeEmpty,
    dotIconColorNotActive,
    dotIconColorActive,
    dotIconColorEmpty,
    dotFontSizeNotActive,
    dotFontSizeActive,
    dotFontSizeEmpty,
    dotTextColorNotActive,
    dotTextColorActive,
    dotTextColorEmpty,
    showStartingJumpDot,
    showEndingJumpDot,
  ...
  let onPressDot = (item) => {
    if (!disableDotOnPressNavigation)
      try {
        props.listRef.scrollToItem(item);
      } catch (e) {
        console.log(" e: ", e);
      }
  ...
  if (StartDot) {
    ...
    onPressDot = (item) => {
      if (!disableStartDotOnPressNavigation)
        try {
          props.listRef.scrollToOffset({ x: 0, y: 0, amimated: true });
        } catch (e) {
          console.log(" e: ", e);
        }
  ...
  return (
    ...
    >
      <View
        style={{
          flexDirection: horizontal !== dotSwapAxis ? "row" : "column",
      ...
      >
        {!dotIconHide && dotPositionSwap && (
          <View style={[dotIconStyle]}>{icon(item)}</View>
        )}
        {!dotTextHide && (
          <Text
            style={[
              {
                textAlign: "center",
                fontWeight:
                  item?.isViewable ?? false === true ? "600" : "500",
